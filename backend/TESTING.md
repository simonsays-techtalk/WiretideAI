...
- Admin token (header `X-Admin-Token`) defaults to `wiretide-admin-dev`; override via `WIRETIDE_ADMIN_TOKEN`.
...
- Approve device (regenerates shared token): `curl -X POST http://127.0.0.1:9000/api/devices/approve -H "X-Admin-Token: wiretide-admin-dev" -H "Content-Type: application/json" -d '{"device_id":<id>,"device_type":"router"}'`
- Queue config for approved device: `curl -X POST http://127.0.0.1:9000/api/queue-config -H "X-Admin-Token: wiretide-admin-dev" -H "Content-Type: application/json" -d '{"device_id":<id>,"package":"wiretide.firewall","package_json":{"profile":"strict"}}'`
- Clear queued configs for a device: `curl -X POST http://127.0.0.1:9000/api/configs/clear -H "X-Admin-Token: wiretide-admin-dev" -H "Content-Type: application/json" -d '{"device_id":<id>}'`
...
## Admin/UI helpers
- List devices (supports filters: `device_type`, `status`, `search`, `limit`, `offset`): `curl "http://127.0.0.1:9000/api/devices?device_type=router&status=approved&search=demo" -H "X-Admin-Token: wiretide-admin-dev"`
- Device detail: `curl http://127.0.0.1:9000/api/devices/<id> -H "X-Admin-Token: wiretide-admin-dev"`
- Get settings: `curl http://127.0.0.1:9000/api/settings -H "X-Admin-Token: wiretide-admin-dev"`
- Regenerate shared token: `curl -X POST http://127.0.0.1:9000/api/settings/token/regenerate -H "X-Admin-Token: wiretide-admin-dev"`
- Update agent update policy: `curl -X PATCH http://127.0.0.1:9000/api/settings/agent-update -H "X-Admin-Token: wiretide-admin-dev" -H "Content-Type: application/json" -d '{"agent_update_policy":"off","agent_update_url":null,"agent_min_version":null}'`
- Toggle monitoring: `curl -X PATCH http://127.0.0.1:9000/api/settings/monitoring -H "X-Admin-Token: wiretide-admin-dev" -H "Content-Type: application/json" -d '{"monitoring_api_enabled":true}'`
- Block device: `curl -X POST http://127.0.0.1:9000/api/devices/block?device_id=<id> -H "X-Admin-Token: wiretide-admin-dev"`
- Remove device: `curl -X DELETE http://127.0.0.1:9000/api/devices/<id> -H "X-Admin-Token: wiretide-admin-dev"`
...
- Browser login (requires `python-multipart`): POST `/login` with `admin_token`, or use `/login` page; sets cookie `wiretide_admin`. `/logout` clears the cookie.
